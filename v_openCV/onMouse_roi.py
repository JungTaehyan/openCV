...
def onMouse(event,x,y,flags,param):     # 마우스 이벤트 핸들 함수
    ...
    if event == cv2.EVENT_LBUTTONDOWN:  # 왼쪽 마우스 버튼 다운, 드래그 시작
        isDragging = True
        x0 = x                          # 마우스 버튼 다운 위치의 x축
        y0 = y                          # 마우스 버튼 다운 위치의 y축

    elif event == cv2.EVENT_MOUSEMOVE:  # 마우스 움직임
        if isDragging:                  # 드래그 진행 중
            img_draw = img.copy()       # 사각형 그림 표현을 위한 이미지 복제
            cv2.rectangle(img_draw, (x0, y0), (x, y), blue, 2) # 드래그 진행 영역 표시, x y는 마우스 위치
        ...
        if isDragging:                  # 드래그 중지
            ...
            w = x - x0                  # 드래그 영역 폭 계산
            h = y - y0                  # 드래그 영역 높이 계산
            print("x:%d, y:%d, w:%d, h:%d" % (x0, y0, w, h)) # roi 시작점 죄표와 폭, 높이
            if w > 0 and h > 0:         # 폭과 높이가 양수이면 드래그 방향이 옳음
                ...
                cv2.imwrite('./imgs/cropped.jpg', roi)   # ROI 영역만 파일로 저장
                print("croped.")
            else:
